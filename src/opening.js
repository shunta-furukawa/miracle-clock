...
let openingDismiss;
export function closeOpening(){openingDismiss?.(false);}
export function showOpening(onDone=()=>{},{onTransition=()=>{},dial=''}={}){
 openingDismiss?.(false);const previous=document.activeElement,root=document.createElement('dialog');root.className='opening-film';root.setAttribute('aria-label','プロローグ — 空のとけい便');
 const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
 root.innerHTML=`<div class="opening-shots" aria-hidden="true">${openingShots.map((s,i)=>`<div class="opening-shot" data-shot="${i}"><div class="opening-camera"><div class="opening-art ${s.cell===undefined?'':'opening-atlas'}" style="background-image:url('assets/${s.asset}');${s.cell===undefined?'':`background-position:${s.cell%3*50}% ${Math.floor(s.cell/3)*100}%;`}"></div>${s.detail? s.detail==='magic-clock.webp'?`<div class="opening-detail opening-clock"><img src="assets/${s.detail}" alt="">${dial}</div>`:`<img class="opening-detail" src="assets/${s.detail}" alt="">`:""}</div></div>`).join('')}</div><div class="opening-light" aria-hidden="true"></div><div class="opening-clouds" aria-hidden="true"></div><div class="opening-dust" aria-hidden="true">${Array.from({length:12},(_,i)=>`<i style="--x:${(i*37+9)%100}%;--delay:${-i*.61}s;--time:${5+i%4}s"></i>`).join('')}</div><div class="opening-vignette" aria-hidden="true"></div><div class="opening-top"><span>PROLOGUE</span><div><button class="opening-pause" aria-label="オープニングを一時停止">Ⅱ</button><button class="opening-skip">スキップ ≫</button></div></div><div class="opening-caption" aria-live="polite" aria-atomic="true"><p></p><h2></h2></div><div class="opening-progress" aria-hidden="true">${openingShots.map(()=>'<i></i>').join('')}</div>`;
 ...
 function finish(run=true){if(closed)return;if(run)transition(.35,1.25);closed=true;cancelAnimationFrame(frame);pan?.cancel();document.removeEventListener('visibilitychange',visibility);root.close();root.remove();openingDismiss=null;if(previous?.isConnected)previous.focus({preventScroll:true});if(run)onDone();}
 ...
 document.addEventListener('visibilitychange',visibility);document.body.append(root);root.showModal();openingDismiss=finish;paint();root.querySelector('.opening-skip').focus({preventScroll:true});
