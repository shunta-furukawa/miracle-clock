...
test('audio races and preference storage never alter adventure data',async()=>{
 ...
 const storage=new Map([['miracle-clock.records.v2',slots]]);
 ...
 try{
  ...
  assert.equal(audio.track,null);assert.equal(storage.get('miracle-clock.records.v2'),slots);
  ...
  assert.equal(audio.configure({soundVolume:.1}),false);assert.equal(storage.get('miracle-clock.records.v2'),slots);
 }finally{Object.assign(globalThis,original)}
});


test('all story chapters and the voyage have distinct music, with safe legacy fallback',()=>{
 assert.equal(new Set([...CHAPTER_MUSIC,'voyage'].map(k=>MUSIC[k].file)).size,7);
 for(let i=0;i<36;i++)assert.equal(gameMusicScene('story',Math.floor(i/6)),CHAPTER_MUSIC[Math.floor(i/6)]);
