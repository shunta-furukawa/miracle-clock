...
try{
 ...
 for(const [name,engine] of [['chromium',chromium],['webkit',webkit]].filter(([n])=>!process.env.TEST_BROWSER||process.env.TEST_BROWSER===n)){
  ...
  assert.equal(decoded.length,26);assert.ok(decoded.every(x=>x.duration>0&&x.peak>.0001),'all recordings decode with non-silent samples');
  await page.locator('#start').click();await page.locator('[data-slot="0"]').click();await page.locator('[data-stage="0"]').click();await page.waitForFunction(()=>soundtrack.track?.key==='forest');await page.evaluate(()=>window.chapterSource=soundtrack.track.source);await page.locator('.story-skip').click();await page.locator('#open-shop').click();assert.equal(await page.evaluate(()=>soundtrack.track.source===window.chapterSource),true,'chapter music continues through conversation and opening');await page.locator('#pause').click();await page.locator('#quit').click();assert.equal(await page.evaluate(()=>soundtrack.track.source===window.chapterSource),true,'chapter music continues on map');await page.locator('[data-stage="0"]').click();await page.locator('.story-skip').click();await page.locator('#open-shop').click();
  await page.waitForFunction(()=>soundtrack.track?.key==='forest');
  await page.locator('#plus').click();assert.equal(await page.evaluate(()=>soundtrack.lastEffect.has('dial-hour')),true,'hour controls use the new tactile sample');await page.locator('[data-select="minute"]').click();assert.equal(await page.evaluate(()=>soundtrack.lastEffect.has('hand-select')),true,'selecting a hand uses its switch sample');
