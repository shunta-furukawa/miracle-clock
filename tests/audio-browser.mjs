...
try{
 ...
 for(const [name,engine] of [['chromium',chromium],['webkit',webkit]].filter(([n])=>!process.env.TEST_BROWSER||process.env.TEST_BROWSER===n)){
  ...
  await page.locator('#start').click();await page.locator('[data-slot="0"]').click();await page.locator('[data-stage="0"]').click();await page.locator('.story-skip').click();await page.locator('#open-shop').click();
