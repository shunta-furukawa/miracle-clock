...
try{
 ...
 for(const [name,engine] of [['chromium',chromium],['webkit',webkit]].filter(([n])=>!process.env.TEST_BROWSER||process.env.TEST_BROWSER===n)){
  ...
  await page.locator('#seal-button').click();assert.equal(await page.evaluate(()=>soundtrack.lastEffect.has('spell')),true,'correct seal plays its dedicated magic recording');await page.locator('#skip-delivery').click();assert.equal(await page.evaluate(()=>document.body.classList.contains('stamping')),false);
