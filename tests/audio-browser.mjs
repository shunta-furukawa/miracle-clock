...
try{
 ...
 for(const [name,engine] of [['chromium',chromium],['webkit',webkit]].filter(([n])=>!process.env.TEST_BROWSER||process.env.TEST_BROWSER===n)){
  ...
  await page.locator('#minute-hand').focus();await page.keyboard.press('ArrowRight');assert.equal(await page.evaluate(()=>soundtrack.lastEffect.has('dial-hour')),true,'hour steps use the new tactile sample');
